...
def _desktop_loopback_auth_exempt(
    ...
    """True for a Desktop-owned loopback backend with a launch credential.

    This is the upstream credential predicate kept as a small compatibility
    helper for callers that only know the bind host. ``start_server`` uses
    ``_is_private_desktop_backend`` below, which additionally requires the
    complete ephemeral-server shape before changing the public-url trust
    boundary.
    """
