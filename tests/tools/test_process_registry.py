...
class TestSystemdCgroupIsolation:
    ...
    @pytest.fixture(autouse=True)
    def _simulate_linux(self, monkeypatch):
        """Exercise the systemd unit paths independently of the test host.

        The production gate is intentionally Linux-only.  Most tests in this
        class mock systemd itself and therefore must also opt into the Linux
        platform branch; the explicit non-Linux tests override this fixture.
        """
